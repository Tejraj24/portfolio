...
import { Github, Globe, ArrowUpRight, Code, Cpu, ExternalLink, Star, GitFork, Clock, X } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-6 right-6 z-40 w-11 h-11 rounded-full bg-neutral-900/90 border border-neutral-800 text-gray-400 hover:text-white hover:bg-black hover:scale-105 hover:rotate-90 transition-all duration-300 flex items-center justify-center interactive-cursor shadow-md"
        ...
        <div className="lg:w-1/2 p-6 md:p-10 lg:p-12 overflow-y-auto flex flex-col justify-between space-y-8 max-h-[50vh] sm:max-h-[45vh] lg:max-h-full scrollbar-thin scrollbar-thumb-neutral-800 scrollbar-track-transparent">
          ...
          <div className="pt-6 border-t border-neutral-900 flex flex-col sm:flex-row gap-4">
            <a
              ...
              className="flex-1 inline-flex items-center justify-center space-x-2 bg-gold-accent text-black font-display font-black uppercase text-xs tracking-widest py-4 px-6 rounded-full hover:bg-white hover:text-black hover:scale-[1.02] transition-all duration-300 shadow-lg interactive-cursor text-center"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center space-x-2 bg-neutral-900 hover:bg-neutral-800 text-white font-mono uppercase text-xs tracking-wider py-4 px-6 rounded-full border border-neutral-800 hover:border-neutral-700 transition-all duration-300 interactive-cursor"
            >
...
function ProjectCardContent({ project, onOpenDetails }: ProjectCardContentProps) {
  ...
  // Map coordinate range 0-1 to rotation angles -8 to 8 for balanced elegant tilt
  const rotateX = useTransform(y, [0, 1], [8, -8]);
  const rotateY = useTransform(x, [0, 1], [-8, 8]);

  // Spring configuration for silky smooth, responsive, anti-jitter physical motion feel
  ...
  const handleMouseLeave = () => {
    // Return gracefully to non-rotated state
    x.set(0.5);
    y.set(0.5);
  };
    ...
    >
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
          ...
          <div className="flex flex-wrap gap-2 bg-neutral-900/60 p-1.5 rounded-full border border-neutral-800" id="projects-filter-container">
            {filterTabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setFilter(tab.id as any)}
                className={`px-4 py-2 rounded-full font-sans text-xs uppercase font-medium tracking-wider transition-all duration-300 interactive-cursor ${
                  filter === tab.id
                    ? 'bg-gold-accent text-black font-semibold shadow-sm'
        ...
        >
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project, index) => {
              // Asymmetrical column assignments based on project qualities
              const isLongCard = project.featured;
        ...
        {/* Dynamic Project Details Dossier Layer */}
        <AnimatePresence>
          {selectedProject && (
            <ProjectDetailModal 
        ...
        <div className="mt-16 text-center">
          ...
          >
            <span>Explore full repository hub</span>
